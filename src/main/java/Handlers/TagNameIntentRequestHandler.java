...
public class TagNameIntentRequestHandler implements RequestHandler
{
    @Override
    public boolean canHandle(HandlerInput input)
    {
        return input.matches(intentName("TagNameIntent"));
    }
        ...
        if (Util.supportsApl(input))
        {
            if (input.getRequestEnvelope().getRequest().getType().equals("IntentRequest"))
            {
                ...
                if (intent != null)
                {
                    ...
                    if (intentName != null)
                    {
                        String tagName = intent.getSlots().get("tag_name").getValue().toLowerCase();
